...
fluxplot = np.array([b1, b2, b3, b4, b5, b6, b7, b8])

dataa = np.array([])
columnss = ['test']
time = np.array([])
dataframe = pd.DataFrame(dataa, index=time, columns=columnss)
        ...
        if len(total_data)>0:
                ...
                if PercentageWINDAvailable>0:
                        ...
                        if np.size(timestamp_temp) != 0:
                                ...
                                date_temp = timestamp_start.strftime("%Y%m%d%H%M%S")
                                datetime_i = datetime.datetime(int(date_temp[0:4]), int(date_temp[4:6]), int(date_temp[6:8]), int(date_temp[8:10]), int(date_temp[10:12]),int(date_temp[12:14]))
