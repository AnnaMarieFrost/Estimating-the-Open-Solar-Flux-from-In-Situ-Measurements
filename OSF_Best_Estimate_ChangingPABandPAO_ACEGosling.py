...
###############################################################################
"""
Using data from solar maximum (year at solar maximum) as used in 
Gosling et al. (1992).
"""
###############################################################################

def FilterElectronMaximum(start_date, end_date):
        start_date = pd.to_datetime(start_date)
        end_date = pd.to_datetime(end_date)
        nc = xr.open_dataset('ACE Electron Corrected Time Solar Maximum One Year.nc')
        swepam_temp = nc.to_dataframe()
        swepam = swepam_temp.iloc[(swepam_temp.index >= start_date) & (swepam_temp.index < end_date)]
        return(swepam)


def FilterMagneticMaximum(start_date, end_date):
        start_date = pd.to_datetime(start_date)
        end_date = pd.to_datetime(end_date)
        nc = xr.open_dataset('ACE Magnetic Corrected Time Solar Maximum One Year.nc')
...
def BodyACE(start_date, end_date): 
        swepam = FilterElectronMaximum(start_date, end_date)
        magswe = FilterMagneticMaximum(start_date, end_date)
        total_data = magswe
...
"""
np.savetxt('TopologyTextFiles/N_AS_ACE_Gosling.txt', N_AS)   
np.savetxt('TopologyTextFiles/N_SS_ACE_Gosling.txt', N_SS)
np.savetxt('TopologyTextFiles/N_CL_ACE_Gosling.txt', N_CL)   
np.savetxt('TopologyTextFiles/N_U_ACE_Gosling.txt', N_U)   
np.savetxt('TopologyTextFiles/OSF_ACE_Gosling.txt', OSF) 
  
np.savetxt('TopologyTextFiles/opposite_ACE_Gosling.txt', opposite)   
np.savetxt('TopologyTextFiles/bgboundary_ACE_Gosling.txt', bgboundary)
